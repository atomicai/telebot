...
from src.llm_bot.api.utils import default_chat_title
from src.llm_bot.db.database import AsyncSession
from src.llm_bot.db.models import Message, MessageTypeEnum, RatingEnum, Thread
...
from src.llm_bot.db.utils import with_async_session
from src.llm_bot.domain.telegram_streaming_handler import TelegramStreamingHandler
...
def get_main_menu_keyboard(
    ...
    if selected_thread:
        ...
        first_button_text = (
            f"✅ {selected_thread.title}" if is_active else f"◻️ {selected_thread.title}"
        )
        buttons = [
            [KeyboardButton(first_button_text), KeyboardButton("💬 Сообщения")],
            [
                KeyboardButton("✏️ Отредактировать"),
                KeyboardButton("🗑️ Удалить"),
            ],
            [KeyboardButton("📜 Чаты"), KeyboardButton("➕ Создать чат")],
        ]
    else:
        buttons = [[KeyboardButton("📜 Чаты"), KeyboardButton("➕ Создать чат")]]
...
async def generate_thread_keyboard(
    user, session, limit=10, offset=0
) -> InlineKeyboardMarkup:
    threads, total = await get_user_threads(
...
async def chat_command(
    ...
    keyboard = await generate_thread_keyboard(
        user, session, limit=10, offset=user.current_thread_offset
    )
...
async def new_chat_command(
    ...
    thread = await create_or_update_thread(
        session, user, title=default_chat_title(), set_active=True
    )
...
async def callback_query_handler(
    ...
    if data == "show_chats":
        ...
        active_thread = await get_active_thread(session, user)
        selected_thread = (
            await get_thread_by_id(session, selected_thread_id)
            if selected_thread_id
            else None
        )
        await query.message.reply_text(
            "Удаление отменено.",
            reply_markup=get_main_menu_keyboard(selected_thread, active_thread),
        )
    elif data.startswith("page_"):
        offset = int(data.split("_")[1])
        keyboard = await generate_thread_keyboard(
            user, session, limit=10, offset=offset
        )
        await query.message.reply_text("Чаты:", reply_markup=keyboard)
...
async def user_message(
    ...
    if text.startswith("✅ ") or text.startswith("◻️ "):
        if selected_thread:
            ...
            if not is_active:
                await create_or_update_thread(
                    session, user, thread_id=selected_thread.id, set_active=True
                )
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "✅ Да",
                                callback_data=f"confirm_delete_{selected_thread.id}",
                            ),
                            InlineKeyboardButton(
                                "❌ Нет", callback_data="cancel_delete"
                            ),
                        ]
                    ]
                ),
    ...
    if "edit_thread_id" in user_data:
        ...
        new_title = text
        await create_or_update_thread(
            session, user, thread_id=thread_id, title=new_title
        )
        thread = await get_thread_by_id(session, thread_id)
    ...
    handler = TelegramStreamingHandler(
        ...
        initial_token_threshold=int(
            kv_dict.get(kv_settings.ai_model_initial_token_threshold_key)
        ),
    ...
    try:
        ...
        try:
            ...
            await update.message.reply_text(
                "Извините, у меня возникли проблемы с генерацией ответа."
            )
            return
    ...
    rating_keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "👍", callback_data=f"rate_{ai_message_db.id}_like"
                ),
                InlineKeyboardButton(
                    "👎", callback_data=f"rate_{ai_message_db.id}_dislike"
                ),
            ]
        ]
    )
