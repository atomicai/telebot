...
async def start(update: Update, context):
    ...
    try:
        ...
        await store.create_back_log(log_data="Received /start command", log_owner="commands.start")
        ...
        if not active_thread:
            active_thread = await store.create_or_update_thread(user_id=user["id"], title=default_chat_title(), set_active=True)
...
async def enable_chat_command(update: Update, context):
    ...
    try:
        ...
        await store.create_back_log(
            log_data="Received command to enable/disable menu (/chat)", log_owner="commands.enable_chat_command"
        )
...
async def new_chat_command(update: Update, context):
    ...
    try:
        ...
        await store.create_back_log(log_data="Received /new_chat command", log_owner="commands.new_chat_command")
...
async def chat_command(update: Update, context):
    ...
    try:
        ...
        await store.create_back_log(log_data="Received /chat command", log_owner="commands.chat_command")
        ...
        keyboard = await keyboard_manager.generate_thread_keyboard(user=user, limit=10, offset=user.get("current_thread_offset", 0))
...
async def callback_query_handler(update: Update, context):
    ...
    try:
        ...
        await store.create_back_log(log_data="Received callback query", log_owner="commands.callback_query_handler")
        ...
        if data.startswith("thread_"):
            ...
            await store.create_back_log(
                log_data=f"Chat selection button pressed: {data}", log_owner="commands.callback_query_handler"
            )
        ...
        if data == "show_chats":
            ...
            await store.create_back_log(log_data="Show chats button pressed", log_owner="commands.callback_query_handler")
            ...
            await store.create_back_log(
                log_data=f"Show history button pressed for thread: {data}", log_owner="commands.callback_query_handler"
            )
            ...
            await store.create_back_log(
                log_data=f"Message rating button pressed: {data}", log_owner="commands.callback_query_handler"
            )
            ...
            try:
                ...
                await store.create_back_log(
                    log_data="Attempt to rate a non-existent message", log_owner="commands.callback_query_handler"
                )
                await query.answer("Сообщение не найдено.")

        elif data.startswith("delete_"):
            logger.info(f"Delete chat button pressed: {data}")
            await store.create_back_log(log_data=f"Delete chat button pressed: {data}", log_owner="commands.callback_query_handler")
            thread_id = data.split("_")[1]
            thread = await store.get_thread_by_id(thread_id)
            user_data["delete_thread_id"] = thread_id

            await query.message.reply_text(
                "Вы уверены, что хотите удалить чат?", reply_markup=keyboard_manager.get_delete_confirmation_keyboard(thread_id)
            )

        elif data.startswith("confirm_delete_"):
            logger.info(f"Chat deletion confirmed: {data}")
            await store.create_back_log(log_data=f"Chat deletion confirmed: {data}", log_owner="commands.callback_query_handler")
            thread_id = data.split("_")[2]
            ...
            await query.message.reply_text(
                "Чат удален.",
                reply_markup=keyboard_manager.get_main_menu_keyboard(context, selected_thread=None, active_thread=active_thread),
            )

        elif data == "cancel_delete":
            logger.info("Delete cancellation button pressed")
            await store.create_back_log(log_data="Delete cancellation button pressed", log_owner="commands.callback_query_handler")
            user_data.pop("delete_thread_id", None)
            ...
            await store.create_back_log(
                log_data=f"Chat pagination button pressed: {data}", log_owner="commands.callback_query_handler"
            )
            offset = data.split("_")[1]
            keyboard = await keyboard_manager.generate_thread_keyboard(user=user, limit=10, offset=offset)
            ...
            await store.create_back_log(log_data="Create new chat button pressed", log_owner="commands.callback_query_handler")
            thread = await store.create_or_update_thread(user["id"], title=default_chat_title(), set_active=True)
            ...
            await store.create_back_log(
                log_data=f"Unknown button/data pressed: {data}", log_owner="commands.callback_query_handler"
            )
            await query.answer()
    finally:
        await store.close()
...
async def user_message(update: Update, context):
    ...
    try:
        ...
        await store.create_back_log(log_data="Received a message from the user", log_owner="commands.user_message")
        ...
        if text.startswith("✅ ") or text.startswith("◻️ "):
            ...
            await store.create_back_log(log_data=f"Chat switch button pressed: '{text}'", log_owner="commands.user_message")
            if selected_thread:
                ...
                await store.create_back_log(log_data="No chat selected while switching.", log_owner="commands.user_message")
                await update.message.reply_text(
                    "Нет выбранного чата.",
                    reply_markup=keyboard_manager.get_main_menu_keyboard(context, selected_thread, active_thread),
                )
            ...
            await store.create_back_log(log_data="Edit button pressed", log_owner="commands.user_message")
            if selected_thread:
                ...
                await update.message.reply_text(
                    ...
                    reply_markup=ReplyKeyboardMarkup([[KeyboardButton("⬅️ Отмена")]], resize_keyboard=True, one_time_keyboard=True),
                )
            else:
                logger.info("Attempted to edit a chat, but no chat was selected.")
                await store.create_back_log(
                    log_data="Attempted to edit a chat, but no chat was selected.", log_owner="commands.user_message"
                )
            ...
            await store.create_back_log(log_data="Delete button pressed", log_owner="commands.user_message")
            if selected_thread:
                user_data["delete_thread_id"] = selected_thread["id"]
                logger.info(f"Sending chat deletion confirmation: {selected_thread['id']}")
                await store.create_back_log(
                    log_data=f"Sending chat deletion confirmation: {selected_thread['id']}", log_owner="commands.user_message"
                )
                await update.message.reply_text(
                ...
                await store.create_back_log(
                    log_data="Attempted to delete a chat, but no chat was selected.", log_owner="commands.user_message"
                )
            ...
            await store.create_back_log(log_data="Messages button pressed", log_owner="commands.user_message")
            if selected_thread:
                ...
                if formatted_messages:
                    ...
                    await store.create_back_log(
                        log_data=f"Sending chat messages {selected_thread['id']} to the user", log_owner="commands.user_message"
                    )
            ...
            await store.create_back_log(log_data="Chats button pressed", log_owner="commands.user_message")
            keyboard = await keyboard_manager.generate_thread_keyboard(
                user=user, limit=10, offset=user.get("current_thread_offset", 0)
            )
            logger.info("Sending chat list to the user")
            await store.create_back_log(log_data="Sending chat list to the user", log_owner="commands.user_message")
            await update.message.reply_text("Чаты:", reply_markup=keyboard)
            return

        elif text == "➕ Новый чат":
            logger.info("New chat button pressed")
            await store.create_back_log(log_data="New chat button pressed", log_owner="commands.user_message")
            thread = await store.create_or_update_thread(user["id"], title=default_chat_title(), set_active=True)
            user_data["selected_thread_id"] = thread["id"]

            logger.info(f"Message to user about creating a new chat: {thread['id']}")
            await store.create_back_log(
                log_data=f"Message to user about creating a new chat: {thread['id']}", log_owner="commands.user_message"
            )
            await update.message.reply_text("Вы создали новый чат")
            await update.message.reply_text(
                f"Активный чат: {thread['title']}",
            ...
            await store.create_back_log(log_data="Cancel button pressed", log_owner="commands.user_message")
            if "edit_thread_id" in user_data:
                user_data.pop("edit_thread_id")
                logger.info("Editing canceled by user")
                await store.create_back_log(log_data="Editing canceled by user", log_owner="commands.user_message")
                await update.message.reply_text(
                ...
                await store.create_back_log(log_data="Canceling another action by user", log_owner="commands.user_message")
                await update.message.reply_text(
                    "Действие отменено.",
                    reply_markup=keyboard_manager.get_main_menu_keyboard(context, selected_thread, active_thread),
                )
        ...
        if "edit_thread_id" in user_data:
            ...
            await store.create_back_log(
                log_data=f"Chat title {thread_id} updated to '{new_title}' by user.", log_owner="commands.user_message"
            )
            await update.message.reply_text(
        ...
        db_messages = await store.get_all_messages_by_thread_id(active_thread["id"], slice=-Config.sgi.LAST_MESSAGES_COUNT * 2)
        ...
        handler = TelegramChatter(
            ...
            edit_interval=int(Config.model.edit_interval),
            initial_token_threshold=int(Config.model.initial_token_threshold),
            typing_interval=int(Config.model.typing_interval),
        )
        ...
        try:
            ...
            await store.create_back_log(
                log_data=f"Error using base_url {Config.model.base_url} LLM: {e}", log_owner="commands.user_message"
            )
            await store.create_pipeline_log(
                message_id=str(human_message["id"]),
            ...
            try:
                llm = ChatOpenAI(
                    model_name=Config.model.openai_default_model,
                    openai_api_key=Config.model.openai_api_key,
                    temperature=float(Config.model.temperature),
                    max_tokens=int(Config.model.max_tokens),
                    streaming=True,
                ...
                await store.create_back_log(log_data=f"Error using fallback LLM: {e2}", log_owner="commands.user_message")
                await store.create_pipeline_log(
                    message_id=str(human_message["id"]),
        ...
        await store.create_back_log(log_data="Saving model response to the database...", log_owner="commands.user_message")
        ...
        try:
            ...
            await store.create_back_log(log_data=f"Error editing message (BadRequest): {e}", log_owner="commands.user_message")
